...
5. For now I am going to give you the code for this in the actions.txt file, but explain what each part does. Go ahead and copy the contents from actions.txt into main.yml.  Feel free to delete actions.txt once this step is complete. P.S. If main.yml gives you an error when you paste the code in, it is okay.  For some reason VSCode doesn't understand yml.<br><br>
    ...
    * Lines 10 - 25: The rest of the lines are unser the steps: subheader.  This just means to perform these actions sequentially.<br><br> ![steps](readme_images/steps.PNG) <br><br>
    * Lines 11-12: Names the step with 'name' and runs the action with 'uses'.  actions/checkout@v2 makes a copy of the code in the repo to use on the ubuntu server that it just started.<br><br> ![checkout](readme_images/checkout.PNG) <br><br>
...
6. Now that we have main.yml set up, we need to make sure we have our dependencies updated  into requirements.txt. This is the part that would be made easier if we used a virtual environment.  We are going to put all of our pip dependencies into a file so that github actions can install them on the server it creates.  If you successfully created a virtual environment, only the dependencies we need are installed, but if you have been using python for a long time and have installed dependencies, our requirements.txt will be bloated and will take longer to run inside of actions.  If this is the case, don't worry, I made a copy of only the dependencies we need and put it into short_requirements.txt.
